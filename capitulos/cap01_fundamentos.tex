...
A homoplasia refere-se ao surgimento independente de estados de caráter similares em linhagens distintas. Diferente da homologia, que é uma relação histórica de identidade derivada de um ancestral comum, a homoplasia resulta de outros processos, como a evolução convergente ou a reversão. Reconhecer homoplasia requer, necessariamente, uma hipótese de relacionamento (uma árvore), porque a determinação de se dois estados de caráter são homólogos ou homoplásticos depende completamente da topologia arbórea adotada.
...
Isso contrasta com práticas tradicionais que frequentemente assumem \textit{a priori} que determinados táxons formam um grupo monofilético (por exemplo, assumir que ``Invertebrados'' é um grupo natural). Na verdade, a monofilia de qualquer agrupamento é uma proposição a ser avaliada pela congruência de caracteres, não uma assunção metodológica.

\subsection{Implementação Prática}

Na prática filogenética moderna, especialmente com dados de sequenciamento de alto rendimento, esta abordagem significa: (1) incluir o máximo possível de táxons, incluindo potenciais grupos externos alternativos; (2) não polarizar caracteres \textit{a priori} com base em comparação com grupo externo; (3) deixar que a análise informe o enraizamento; (4) testar múltiplas topologias para verificar se a posição de certos táxons é bem suportada.
...
A sistemática filogenética requer um critério de otimalidade: uma métrica pela qual comparamos árvores candidatas e selecionamos aquela(s) que melhor explicam os dados. Os critérios cladísticos atualmente (Março, 2026) disponíveis são:
...
\begin{itemize}
    ...
    \item \textbf{PMDL (\textit{phylogenetic minimum description length})}: o custo computacional mínimo, medido em bytes (espaço de disco ocupado para armazenamento), do comprimento da descrição da árvore mais o comprimento da descrição dos dados, dada a árvore \parencite{wheeler2025}. Veja mais detalhes no Capítulo 7, ``Tecnologia da Informação Aplicada a Problemas Filogenéticos''.
\end{itemize}

Cada critério encarna diferentes assunções filosóficas e representa uma hipótese distinta sobre como a inferência deve proceder.
...
\begin{enumerate}
    ...
    \item Homoplasia = erro de codificação, causado por semelhança decorrente de processos que poderiam ser convergência ou reversão; só é detectável \textit{dado} um cladograma.
    ...
    \item Otimização Direta (\textit{Direct Optimization}) \parencite{wheeler2006} co-otimiza alinhamento e árvore sob critério de otimalidade.
    \item Quatro critérios de otimalidade: parcimônia --- mínimo número de transformações ou homoplasias; ML --- $\max \Pr(D|T,\theta)$ --- ; inferência Bayesiana --- $P(T|D)$ --- ; e PMDL (\textit{phylogenetic minimum description length}) --- $(\text{comprimento de descrição da árvore}) + (\text{comprimento de descrição dos dados } | \text{ árvore})$.
