...
The recognition that eukaryotic genomes are dominated by repetitive sequences dates to the reassociation kinetics experiments of \textcite{britten1968}, who demonstrated that a large fraction of animal DNA consists of reiterated sequences. Since then, whole-genome sequencing has confirmed that repetitive elements account for substantial proportions of genome size across the tree of life.
...
LTR retrotransposons are structurally related to retroviruses; indeed, endogenous retroviruses (ERVs) are considered a subclass of LTR retrotransposons that likely originated from ancestral germline retroviral infections \parencite{jern2008}. Non-LTR retrotransposons integrate via target-primed reverse transcription (TPRT), a mechanism in which the element's endonuclease nicks the target DNA and uses the exposed 3$'$ hydroxyl as a primer for reverse transcription \textit{in situ} \parencite{luan1993}. Additionally, YR retrotransposons represent a third category that shares reverse transcriptase homology with LTR retrotransposons but possesses a tyrosine recombinase domain more closely related to those found in certain DNA transposons \parencite{goodwin2003}, illustrating the modular and chimeric nature of TE evolution.
...
\subsection{Interplay Between Repeat Classes}

The traditional classification of repeats into interspersed and tandem classes belies extensive reciprocal interactions between these compartments. Transposable elements can serve as substrates for the \textit{de novo} emergence of satellite DNA families: when TE copies are clustered, unequal crossing over and recombination-based amplification can expand them into tandem arrays, effectively converting interspersed repeats into satellite DNA \parencite{mestrovic2015}. Conversely, TEs accumulate preferentially within satellite-rich heterochromatic regions, where they may be silenced but contribute to heterochromatin dynamics.

The life cycle of a TE lineage can be described in four phases: invasion (horizontal transfer or activation of a previously dormant element), amplification (proliferation throughout the host genome), maturation and silencing (host defense mechanisms such as DNA methylation and piRNA pathways suppress transposition), and death or degeneration (accumulation of mutations renders the element non-functional). This four-phase model applies across diverse TE superfamilies and host lineages, though the duration and intensity of each phase vary considerably \parencite{kidwell2002}.
...
MotifScope represents a newer algorithmic approach for characterization and visualization of tandem repeat motifs, identifying more motifs and more accurately representing underlying repeat sequences than previous methods \parencite{zhang2025}.
...
Initial analyses have demonstrated substantial improvements in variant discovery, with a 34\% reduction in small variant errors and a 104\% increase in the detection of structural variants per haplotype compared to GRCh38-based workflows \parencite{liao2023}. The most recent data release incorporates phased haplotypes from 232 individuals, selected to represent diverse ancestries, with the goal of expanding to over 700 haploid genomes \parencite{wang2022}. This population-level approach has yielded an average of over 29,000 structural variants per individual, compared to fewer than 16,000 when using a linear reference alone \parencite{groza2024}.
...
Despite the transformative advances in long-read sequencing and pangenome construction, several challenges remain. The cost and complexity of generating multiple complementary data types (PacBio HiFi, ONT ultra-long, Strand-seq) for a single genome remain prohibitive for many non-model organisms. Computational resources required for pangenome graph construction and variant calling at population scale are substantial. Multiallelic VNTRs present particular difficulties for pangenome graphs, as their combinatorial complexity can make graph representations intractable.

At the single-cell level, the convergence of long-read sequencing with single-cell omics is opening new avenues for understanding repeat element dynamics, including somatic retrotransposition and allele-specific expression of TE-derived regulatory elements. The development of tools like RAmbler, which resolves complex repeats in specific human chromosomes \parencite{chakravarty2025}, and improvements in long-read RNA sequencing methods further extend the analytical toolkit for repeat biology.

The adoption of the T2T-CHM13v2.0 assembly as the standard reference, together with the maturation of the HPRC pangenome, will provide a more complete foundation for understanding repeat biology and its implications for genome evolution, gene regulation, and human disease.
...
\begin{enumerate}
    ...
    \item Interplay between repeat classes: TEs serve as substrates for \textit{de novo} satellite DNA emergence via unequal crossing over; four-phase TE life cycle (invasion, amplification, silencing, degeneration).
    \item Emerging challenges: cost of multi-technology sequencing, pangenome computational requirements, multiallelic VNTR complexity; single-cell long-read approaches for somatic TE dynamics; T2T-CHM13v2.0 as standard reference.
\end{enumerate}
\end{revise}
